...
class EncoderRNN(nn.Module):
    def __init__(self, input_size,batch_size, hidden_size):
        ...
        self.gru = nn.GRU(hidden_size,batch_size, hidden_size)
...
def load_src_data():
    ...
    return (encode[:train_length], decode[:train_length]),\
        (encode[train_length:train_length+test_length], decode[train_length:train_length+test_length]), \
        vec
...
def training(data,eval_data):
    ...
    for epoch in range(config.epoch):
        ...
        total_loss = 0
       # print('\nDATA',data)
        output,loss  = model(data[0,:,:],data[1,:,:])
