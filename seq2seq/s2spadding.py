...
class DecoderRNN(nn.Module):
    ...
    def initHidden(self):
        return torch.uniform_(1,1, self.hidden_size, device=device)
...
(data_train, trg_train),(data_test,trg_test), vec = load_src_data(5)
...
cat_training10 = torch.cat((data_train10.unsqueeze(0),trg_train10.unsqueeze(0)),dim=0)
cat_test10 =  torch.cat((data_test10.unsqueeze(0),trg_test10.unsqueeze(0)),dim=0)

cat_training50 = torch.cat((data_train50.unsqueeze(0),trg_train50.unsqueeze(0)),dim=0)
cat_test50 =  torch.cat((data_test50.unsqueeze(0),trg_test50.unsqueeze(0)),dim=0)
...
training(torch.transpose(cat_training10,0,1),torch.transpose(cat_test10,0,1))
print('padding=50----')

training(torch.transpose(cat_training50,0,1),torch.transpose(cat_test50,0,1))